...
import type { AIInsight, DailyTarget, DashboardStats, Quote } from '@/types';
...
export function RightPanel({ stats }: RightPanelProps) {
  ...
  useEffect(() => {
    quotesApi.random().then((r) => setQuote(r.data)).catch(() => {});
    aiApi.insights().then((r) => setInsights(r.data.slice(0, 3))).catch(() => {});
    studyApi.targets().then((r) => setTargets(r.data)).catch(() => {});
  }, []);

  const refreshQuote = () => {
    quotesApi.random().then((r) => setQuote(r.data));
  };
  ...
  return (
    <aside className="hidden xl:flex flex-col gap-4 w-[300px] shrink-0">
      ...
      <GlassCard className="!p-4">
        ...
        <motion.div className="grid grid-cols-3 gap-2 mt-3 text-center text-xs">
          {[
            { label: 'Mock', val: stats?.mock_streak ?? 0 },
            { label: 'Revision', val: stats?.revision_streak ?? 0 },
            { label: 'Focus', val: stats?.focus_streak ?? 0 },
          ].map((s) => (
            <motion.div key={s.label} whileHover={{ scale: 1.05 }} className="bg-white/5 rounded-lg py-2">
          ...
          >
            <GlassCard className="!p-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <button onClick={refreshQuote} className="text-slate-500 hover:text-white">
                  <RefreshCw size={14} />
                </button>
              </div>
              <p className="text-sm italic text-slate-300 leading-relaxed">"{quote.text}"</p>
      ...
      <GlassCard className="!p-4">
        <motion.div className="flex items-center justify-between mb-3">
          ...
          <button onClick={generateAI} className="text-xs text-blue-400 hover:underline">
            Generate
          </button>
        </motion.div>
